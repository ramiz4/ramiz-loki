export const translations = {
  en: {
    // Navbar
    nav: {
      about: 'ABOUT',
      skills: 'SKILLS',
      experience: 'EXPERIENCE',
      education: 'EDUCATION',
      contact: 'CONTACT',
    },
    // Header
    header: {
      welcome: 'WELCOME TO MY PORTFOLIO',
      name: 'Ramiz',
      title:
        'Senior Full-Stack Software Engineer crafting elegant solutions to complex problems',
      contactMe: 'Contact Me',
      viewSkills: 'View My Skills',
    },
    // About
    about: {
      title: 'About Me',
      subtitle: 'My experience, expertise and passion',
      fullStackTitle: 'Full-Stack Developer',
      fullStackDesc:
        'Experienced in building responsive web applications using modern frameworks and technologies.',
      teamLeaderTitle: 'Team Leader',
      teamLeaderDesc:
        'Led the shift to agile, introducing iterative delivery methods as a team thought leader.',
      problemSolverTitle: 'Problem Solver',
      problemSolverDesc:
        'Passionate about following best practices and participating in code reviews to ensure high-quality code.',
    },
    // Skills
    skills: {
      title: 'Skills & Expertise',
      subtitle:
        "My technical toolkit that I've built and refined over the years of development journey",
      all: 'All',
      frontend: 'Frontend',
      backend: 'Backend',
      database: 'Database',
      tools: 'Tools',
    },
    // Experience
    experience: {
      title: 'Professional Experience',
      subtitle: 'My journey through various roles and organizations',
      present: 'Present',
      currentlyWorking:
        'Currently working in this position. More details to come...',
    },
    // Education
    education: {
      title: 'Education',
      subtitle: 'My academic background and certifications',
    },
    // Contact
    contact: {
      getInTouch: 'GET IN TOUCH',
      title: 'Contact',
      me: 'Me',
      subtitle: "Let's discuss your project or opportunities to work together",
      sendMessage: 'Send a Message',
      yourName: 'Your Name',
      emailAddress: 'Email Address',
      subject: 'Subject',
      message: 'Message',
      sendButton: 'Send Message',
      successMessage: "Message sent successfully! I'll get back to you soon.",
      footerNote:
        'You can also find all my contact information in the footer below.',
      namePlaceholder: 'John Doe',
      emailPlaceholder: 'john@example.com',
      subjectPlaceholder: 'Project Inquiry',
      messagePlaceholder: 'Your message here...',
    },
    // Footer
    footer: {
      tagline:
        'Senior Full-Stack Software Engineer with extensive experience in web development and team leadership.',
      contactInfo: 'Contact Info',
      email: 'Email',
      phone: 'Phone',
      location: 'Location',
      allRightsReserved: 'All rights reserved.',
    },
  },
  de: {
    // Navbar
    nav: {
      about: 'ÜBER MICH',
      skills: 'FÄHIGKEITEN',
      experience: 'ERFAHRUNG',
      education: 'BILDUNG',
      contact: 'KONTAKT',
    },
    // Header
    header: {
      welcome: 'WILLKOMMEN IN MEINEM PORTFOLIO',
      name: 'Ramiz',
      title:
        'Senior Full-Stack Software-Ingenieur, der elegante Lösungen für komplexe Probleme entwickelt',
      contactMe: 'Kontaktieren Sie mich',
      viewSkills: 'Meine Fähigkeiten ansehen',
    },
    // About
    about: {
      title: 'Über mich',
      subtitle: 'Meine Erfahrung, Expertise und Leidenschaft',
      fullStackTitle: 'Full-Stack-Entwickler',
      fullStackDesc:
        'Erfahren im Erstellen responsiver Webanwendungen mit modernen Frameworks und Technologien.',
      teamLeaderTitle: 'Teamleiter',
      teamLeaderDesc:
        'Leitete den Übergang zu Agile und führte iterative Bereitstellungsmethoden als Vordenker des Teams ein.',
      problemSolverTitle: 'Problemlöser',
      problemSolverDesc:
        'Leidenschaftlich daran interessiert, Best Practices zu befolgen und an Code-Reviews teilzunehmen, um qualitativ hochwertigen Code sicherzustellen.',
    },
    // Skills
    skills: {
      title: 'Fähigkeiten & Expertise',
      subtitle:
        'Mein technisches Toolkit, das ich im Laufe meiner Entwicklungsreise aufgebaut und verfeinert habe',
      all: 'Alle',
      frontend: 'Frontend',
      backend: 'Backend',
      database: 'Datenbank',
      tools: 'Werkzeuge',
    },
    // Experience
    experience: {
      title: 'Berufserfahrung',
      subtitle: 'Meine Reise durch verschiedene Rollen und Organisationen',
      present: 'Heute',
      currentlyWorking:
        'Derzeit in dieser Position tätig. Weitere Details folgen...',
    },
    // Education
    education: {
      title: 'Bildung',
      subtitle: 'Mein akademischer Hintergrund und Zertifizierungen',
    },
    // Contact
    contact: {
      getInTouch: 'KONTAKTIEREN SIE MICH',
      title: 'Kontakt',
      me: 'mich',
      subtitle:
        'Lassen Sie uns über Ihr Projekt oder Möglichkeiten zur Zusammenarbeit sprechen',
      sendMessage: 'Eine Nachricht senden',
      yourName: 'Ihr Name',
      emailAddress: 'E-Mail-Adresse',
      subject: 'Betreff',
      message: 'Nachricht',
      sendButton: 'Nachricht senden',
      successMessage:
        'Nachricht erfolgreich gesendet! Ich werde mich bald bei Ihnen melden.',
      footerNote:
        'Sie finden auch alle meine Kontaktinformationen in der Fußzeile unten.',
      namePlaceholder: 'Max Mustermann',
      emailPlaceholder: 'max@beispiel.de',
      subjectPlaceholder: 'Projektanfrage',
      messagePlaceholder: 'Ihre Nachricht hier...',
    },
    // Footer
    footer: {
      tagline:
        'Senior Full-Stack Software-Ingenieur mit umfangreicher Erfahrung in Webentwicklung und Teamleitung.',
      contactInfo: 'Kontaktinformationen',
      email: 'E-Mail',
      phone: 'Telefon',
      location: 'Standort',
      allRightsReserved: 'Alle Rechte vorbehalten.',
    },
  },
};

export type TranslationKeys = typeof translations.en;
